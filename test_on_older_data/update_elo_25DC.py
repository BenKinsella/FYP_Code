...
Switch between datasets by changing DATASET at the top:
  DATASET = 1  →  results1 / elo1_dc   (your existing pipeline)
  DATASET = 2  →  results2 / elo2_dc   (football-data.co.uk CSV)

For DATASET = 1, initial Elos are loaded from CSV_PATH.
For DATASET = 2, initial Elos are loaded from initial_elos2 in the DB.
rho is calibrated separately per dataset.
"""
...
HFA        = 42
K          = 20
RHO        = 0.1   # overwritten by calibration
...
ELO_TABLE     = "elo1_dc"   if DATASET == 1 else "elo2_dc"
...
def fetch_draw_params(conn) -> tuple:
    """Pull shared draw_base and draw_slope from the relevant analysis base table."""
...
def main():
    ...
    try:
        ...
        # 5. Run engine with calibrated rho
        engine          = FootballEloDC(initial_elos, rho=rho)
