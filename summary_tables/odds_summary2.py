...
def main():
    ...
    try:
        print("Fetching data...")
        elo_base    = fetch_elo_lookup(conn, "elo1")
        elo_hfa     = fetch_elo_lookup(conn, "elo1_hfa")
        elo_decay   = fetch_elo_lookup(conn, "elo1_decay")
        elo_dc      = fetch_elo_lookup(conn, "elo1_dc")
        print(f"  Base Elo lookup        : {len(elo_base)} entries.")
        print(f"  Dynamic-HFA Elo lookup : {len(elo_hfa)} entries.")
        print(f"  Time-Decay Elo lookup  : {len(elo_decay)} entries.")
        print(f"  Dixon-Coles Elo lookup : {len(elo_dc)} entries.")
        ...
        for _, row in df.iterrows():
            ...
            rows.append((
                ...
                round(base_o[0],4),  round(base_o[1],4),  round(base_o[2],4),
                round(hfa_o[0],4),   round(hfa_o[1],4),   round(hfa_o[2],4),
                round(decay_o[0],4), round(decay_o[1],4), round(decay_o[2],4),
                round(dc_o[0],4),    round(dc_o[1],4),    round(dc_o[2],4),
                round(pin_o[0],4),   round(pin_o[1],4),   round(pin_o[2],4),

                # ── Closing probabilities: all five models ──
                round(base_c[0],4),  round(base_c[1],4),  round(base_c[2],4),
                round(hfa_c[0],4),   round(hfa_c[1],4),   round(hfa_c[2],4),
                round(decay_c[0],4), round(decay_c[1],4), round(decay_c[2],4),
                round(dc_c[0],4),    round(dc_c[1],4),    round(dc_c[2],4),
                round(pin_c[0],4),   round(pin_c[1],4),   round(pin_c[2],4),

                # ── Information gain: open then close, all five models ──
                round(ig(base_o[idx]),4),  round(ig(hfa_o[idx]),4),
                round(ig(decay_o[idx]),4), round(ig(dc_o[idx]),4),
                round(ig(pin_o[idx]),4),
                round(ig(base_c[idx]),4),  round(ig(hfa_c[idx]),4),
                round(ig(decay_c[idx]),4), round(ig(dc_c[idx]),4),
                round(ig(pin_c[idx]),4),
